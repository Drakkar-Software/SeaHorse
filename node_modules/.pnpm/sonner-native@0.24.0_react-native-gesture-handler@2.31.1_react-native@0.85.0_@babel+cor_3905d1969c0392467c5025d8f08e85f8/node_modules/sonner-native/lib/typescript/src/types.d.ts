...
type ExternalToast = Omit<ToastProps, 'id' | 'type' | 'title' | 'jsx' | 'promise' | 'variant'> & {
    id?: string | number;
};
...
export type AddToastContextHandler = (data: Omit<ToastProps, 'id'> & {
    id?: string | number;
}) => string | number;
export type ToasterContextType = Required<Pick<ToasterProps, 'duration' | 'swipeToDismissDirection' | 'closeButton' | 'position' | 'invert' | 'icons' | 'offset' | 'pauseWhenPageIsHidden' | 'gap' | 'theme' | 'toastOptions' | 'autoWiggleOnUpdate' | 'richColors'>> & {
    addToast: AddToastContextHandler;
};
export declare const toast: ((message: string, data?: ExternalToast) => string | number) & {
    ...
    promise: <T>(promise: Promise<T>, options: Omit<PromiseOptions, 'promise'>) => string | number;
